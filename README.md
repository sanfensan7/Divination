...
- **v1.0.3**：等待提示弹窗现在展示本地真太阳时排盘（打字机样式），并使用全屏遮罩阻止底层表单误触。
- **v1.0.2**：新增本地模拟模式与故障排查指引，DeepSeek 调用失败会自动降级并保留日志，便于定位网络/密钥问题。
- **v1.0.1**：补充极简风格动画体系（按钮压感、列表渐入、页面转场、骨架屏），提升交互流畅度并适配“减少动画”。
- **v1.0.0**：首个公开版本，提供多算命方式入口、MBTI/易经/塔罗等基础题库与结果展示，支持 API Key 配置与设置页。
